...
export function ChestIntroScreen({
  ...
  const [isChestVisible, setIsChestVisible] =
    useState(false);
  ...
  return (
    ...
    >
      {/* Filmové pozadie */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_50%_58%,rgba(146,64,14,0.20),transparent_38%),linear-gradient(to_bottom,#010207,#050307_62%,#000)]" />

      {/* Vineta */}
      <div className="pointer-events-none absolute inset-0 shadow-[inset_0_0_150px_65px_rgba(0,0,0,0.95)]" />

      {/* Hlavná žiara */}
      <div
        className={`pointer-events-none absolute left-1/2 top-[58%] h-[38rem] w-[38rem] -translate-x-1/2 -translate-y-1/2 rounded-full bg-amber-500/10 blur-3xl transition-all duration-[2800ms] ease-out ${
          isStarted
      ...
      {/* Svetelné jadro */}
      <div
        className={`pointer-events-none absolute left-1/2 top-[60%] h-64 w-[28rem] -translate-x-1/2 -translate-y-1/2 rounded-[50%] bg-amber-200/15 blur-3xl transition-all duration-[2600ms] ease-out ${
          isChestVisible
      ...
      {/* Jemná atmosférická hmla za truhlicou */}
      <div
        className={`pointer-events-none absolute left-1/2 top-[55%] h-72 w-[42rem] max-w-[92vw] -translate-x-1/2 -translate-y-1/2 rounded-[50%] bg-amber-500/10 blur-[72px] transition-all duration-[3000ms] ease-out ${
          isStarted
      ...
      {/* Mäkký svetelný odraz pod truhlicou */}
      <div
        className={`pointer-events-none absolute left-1/2 top-[67%] h-28 w-[30rem] max-w-[78vw] -translate-x-1/2 rounded-[50%] bg-amber-300/12 blur-3xl transition-all duration-[2800ms] ease-out ${
          isChestVisible
            ? "scale-100 opacity-100"
            : "scale-75 opacity-0"
        }`}
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        {particles.map((particle, index) => (
          <span
            ...
            className={`absolute rounded-full bg-amber-100 shadow-[0_0_12px_rgba(253,230,138,0.95)] transition-all ease-out ${
              isStarted
                ? "-translate-y-40 opacity-70"
      ...
      <div className="relative z-10 mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-lg flex-col justify-center">
        ...
        >
          ...
          <h1 className="mt-5 font-serif text-2xl leading-tight text-amber-50 drop-shadow-[0_0_24px_rgba(251,191,36,0.3)] sm:text-3xl">
            Všetky stopy smerovali k jedinému
            tajomstvu.
          </h1>

          <p className="mx-auto mt-4 max-w-sm text-sm leading-7 text-slate-300">
            Mechanizmus celé desaťročia čakal na
            niekoho, kto dokáže spojiť všetky
            získané dôkazy.
          </p>
        ...
        {/* Skutočná 3D truhlica */}
        <div
          className={`relative -mx-4 mt-2 transition-all duration-[2400ms] ease-out sm:-mx-12 ${
            isChestVisible
              ? "translate-y-0 scale-100 opacity-100 blur-0"
              : "translate-y-16 scale-[0.72] opacity-0 blur-xl"
          }`}
        >
          {/* Počas cinematicu truhlica ešte nereaguje na dotyk */}
        ...
        {/* Cinematic sa tu zastaví a čaká na kliknutie */}
        <div
        ...
        >
          ...
          <p className="mt-3 text-center text-xs leading-5 text-slate-400">
            Po otvorení pohľadu môžeš truhlicu
            otáčať a preskúmať jej mechanizmus.
          </p>
        </div>
      </div>
