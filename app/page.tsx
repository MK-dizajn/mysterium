...
import type { ActId, Artifact, GameScreen } from "../types/game";
...
export default function Home() {
  ...
  const fallbackChapter = mysteriumGame.chapters[0];
  ...
  useEffect(() => {
    ...
    gameState.currentActId,
  ]);

  useEffect(() => {
    const chapterExists = Boolean(
      mysteriumGame.chapters[gameState.currentChapterIndex]
    );

    const sceneExists = Boolean(
      chapterExists &&
        mysteriumGame.chapters[gameState.currentChapterIndex]?.scenes[
          gameState.currentSceneIndex
        ]
    );
  ...
  function openInventory() {
    setGameState((currentState) => ({
      ...currentState,
      returnScreen: currentState.screen,
      screen: "inventory",
    }));
  }
  ...
  function closeCollection() {
    setGameState((currentState) => ({
      ...currentState,
      screen: currentState.returnScreen ?? "puzzle",
  ...
  function continueToNextAct(
    ...
    setGameState((currentState) => {
      ...
      const alreadyCollected = currentState.inventory.some(
          ...
          onSolved={(hintsUsed) => {
            setGameState((currentState) => {
              ...
              if (newlyUnlockedArtifact) {
                setUnlockedArtifact(newlyUnlockedArtifact);
              }
